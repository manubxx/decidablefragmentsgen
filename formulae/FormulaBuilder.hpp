...
public:
    ...
    virtual std::string generateFormatted(const GenConfig& cfg);
    virtual std::string fragmentName() const = 0;
...
protected:
    ...
    struct BudgetState {
        ...
        bool canUse(SymbolType t) const {
            switch (t) {
        ...
        bool satisfied() const { return remaining() == 0; }

    };

    // Pure virtual methods
    virtual std::unique_ptr<AtomicNode> buildAtomic(const std::string& currentVar) = 0;

    virtual std::unique_ptr<ASTNode> generateSAT(int depth, int domainSize, BudgetState& budget) = 0;

    virtual std::unique_ptr<ASTNode> buildComponentUNSAT(int depth, BudgetState& budget) = 0; 

    

    virtual std::string startVar() const = 0;
    virtual std::string nextVar(const std::string& currentVar) const = 0;
